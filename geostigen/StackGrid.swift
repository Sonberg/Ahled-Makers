...
class StackGrid : UIView {
    ...
    private var viewsToDisplay = [UIView]() {
        didSet {
            if viewsToDisplay.count >= 0 {
                buildTreeForNodeCount(count: viewsToDisplay.count)
                updateViews()
            }
        }
    }
    ...
    */
    ...
    private func buildTreeForNodeCount(count: Int) {
        ...
        //attach leaves
        let firstLeafLayer = getLayerForIndex(index: count) //count = first leaf
        ...
        if firstLeafLayer != secondLeafLayer {//only if layers are different, else skip right to forwards fill up!
            ...
            for i in tree.indices.reversed() where tree[i].type == .Leaf{
                ...
                for j in tree.indices.reversed() where (tree[j].type == .Node) && (getLayerForIndex(index: j) == firstNodeLayer) {
        ...
        for i in 0..<tree.count where (tree[i].type == .Leaf) && !tree[i].set {
            ...
            for j in tree.indices where (tree[j].type == .Node) && (getLayerForIndex(index: j) == secondNodeLayer) {
    ...
    private func updateViews() {
        ...
        if viewsToDisplay.count == 0 {
            for view in views where !(view is UIStackView) { //remove all leaves from superviews
    ...
    private func allLeavesSameLayer() -> Bool {
        ...
        for i in tree.indices where tree[i].type == .Leaf {
            if !set {
                layer = getLayerForIndex(index: i)
                set = true
            }else{
                if getLayerForIndex(index: i) != layer {
                    return false
                }
            }
        }
    ...
    private func getNodeViewCount() -> Int {
        var count = 0
        for view in views where view is UIStackView  {
            count += 1
        }
        return count
    }
    ...
    */
    ...
    private func printTree(){
        for i in tree.indices {
            ...
            print("\(getIDStringForIndex(index: i)), index: \(i), type: \(tree[i].type)")
            
            if let child1 = tree[i].child1ViewIndex {
                print(" Child 1: \(getIDStringForIndex(index: child1)), index: \(child1)")
            }
            
            if let child2 = tree[i].child2ViewIndex {
                print(" Child 2: \(getIDStringForIndex(index: child2)), index: \(child2)")
            }
        }
    }
    ...
    private func printStackViewStructureWithPrefix(prefix: String, views: [UIView]){
        for view in views{
            if view is UIStackView{
                ...
                printStackViewStructureWithPrefix(prefix: prefix + "-", views: subContainer.arrangedSubviews)
            }else{
