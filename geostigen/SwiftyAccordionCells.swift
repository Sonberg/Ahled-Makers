...
class SwiftyAccordionCells {
    ...
    func append(_ item: Item) {
        if item is SwiftyAccordionCells.Item {
            FIRDatabase.database().reference().child("users").child(item.value).observe(.value, with: { (snap : FIRDataSnapshot) in
                if snap.exists() {
                    let user = User(snap: snap)
                    item.value = String(user.firstName + " " + user.lastName)
